...
export default function ContentDetail() {
  ...
  useEffect(() => {
    if (content?.fileVersions && selectedVersion) {
      const version = content.fileVersions.find(
  ...
  const handleDownload = () => {
    ...
    if (fileVersion.fileUrl) {
      window.open(fileVersion.fileUrl, "_blank");
  ...
  const handleDelete = () => {
    ...
    if (success) {
      ...
      toast({
        title: "Ошибка",
        description:
          "Не удалось удалить материал. У вас может не быть прав на это действие.",
        variant: "destructive",
      });
    }
          ...
          <div className="space-y-4">
            ...
            {content.fileVersions && content.fileVersions.length > 0 ? (
                  ...
                  <SelectContent>
                    {content.fileVersions.map((version) => (
                      <SelectItem key={version.version} value={version.version}>
                        {version.version}{" "}
                        {version.fileSize && `(${version.fileSize})`}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                ...
                {fileVersion && (
                  <div className="text-sm text-muted-foreground">
                    {fileVersion.fileName && (
                      <p>Файл: {fileVersion.fileName}</p>
                    )}
                    {fileVersion.fileSize && (
                      <p>Размер: {fileVersion.fileSize}</p>
                    )}
                  </div>
                )}
              </div>
              ...
              <p className="text-amber-600 text-sm">
                Для этого материала не добавлены файлы
              </p>
            )}
              ...
              >
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Это действие нельзя отменить. Материал и все его файлы
        ...
        <div className="md:col-span-2">
          ...
          <Card className="mb-8">
            <CardContent className="p-6">
              ...
              {content.minecraftVersions &&
                content.minecraftVersions.length > 0 && (
                  <div className="mt-4">
